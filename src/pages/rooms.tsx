...
export default function Rooms(props: any) {
  ...
  const validRoomTypes = ["Cozy Comfort Suite", "Scenic View Suite", "Culinary Retreat Suite", "Trio Harmony Suite"];
 ...
 // Fetch available rooms
  const { data: rooms, error, isLoading } = trpc.room.findAvailableRooms.useQuery({
    ...
    const reducer = (state: any, action: any) => {
      switch (action.type) {
        case actionTypes.UPDATE_ROOM_DETAILS: {
          const { roomType, totalRooms, price } = action.payload;
          return {
            ...state,
            roomsData: {
              ...state.roomsData,
    ...
    useEffect(() => {
      if (rooms && rooms.availableRooms && rooms.roomTypes) {
        rooms.availableRooms.forEach(room => {
          dispatch({
            type: actionTypes.UPDATE_ROOM_DETAILS,
            payload: {
    ...
    useEffect(() => { // allocate and update room counts based on guest numbers and requested rooms, optimise to meet both capacity and request constraints.
        ...
        interface RoomCapacities {
          ...
          const roomCapacities: RoomCapacities = {
              "Cozy Comfort Suite": initialState.roomsData["Cozy Comfort Suite"].capacity,
              "Scenic View Suite": initialState.roomsData["Scenic View Suite"].capacity,
              "Culinary Retreat Suite": initialState.roomsData["Culinary Retreat Suite"].capacity,
              "Trio Harmony Suite": initialState.roomsData["Trio Harmony Suite"].capacity,
          };
        ...
        // Now we ensure that we only dispatch for actual room types
...
return (
      <>
        ...
        ) : (!rooms || !rooms.availableRooms || !rooms.roomTypes) ? (
          // Display a message if no data is available
          <div>No data available</div>
        ) : (
          // Render the main component UI if data is available
          <div>
            ...
            <div className="container mx-auto">
              ...
              <div className="grid md:grid-cols-2 gap-4 my-8">
                ...
                {Object.entries(state.roomsData)
                .filter(([roomType]) => validRoomTypes.includes(roomType)) // Only include valid room types
                .map(([roomType, roomDetails]) => (
                  <RoomSection
                    key={roomType}
                    roomData={{
                      name: roomType,
                ...
                <div className="md:mx-2 mx-10 mt-8">
                  <div className="bg-white rounded md:py-8 py-4 px-4">
                    ...
                    <div className="flex flex-wrap -m-2">
                      ...
                      <div className="p-2 w-full">
                        <button className="flex mx-auto text-white bg-orange-900 border-0 py-2 px-8 focus:outline-none hover:bg-orange-700 rounded text-lg">Continue To Payment</button>
                      </div>
                    </div>
